...
def monsterax_search(query, MAX_RESULTS, STRICT_SEARCH):
  ...
  for i in range(len(res_end)):
    ...
    # flags and corrects trailing symbols on price
    ERROR_FLAG = False
    
    if(STRICT_SEARCH):
      if query in curr:
        # double check for symbols
        for c in curr[:num.start()-1]:
          if not c.isnumeric():
            parsed_price = locale.atof(curr[:num.start()-2])
            listings.append(Listing(curr[:num.start()-2], curr[num.start():]))
            ERROR_FLAG = True
        if ERROR_FLAG == False:
